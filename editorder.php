...
// Steps for btnupdateorder button
 ...
 foreach($row_invoice_details as $item_invoice_details){
  $updateproduct = $pdo->prepare("UPDATE tbl_product SET
  pstock=pstock+".$item_invoice_details['qty']." WHERE 
  pid = '".$item_invoice_details['product_id']."'");
...
$delete_invoice_details->execute();
 ...
 //4) Write update query for tbl_invoice table data
  ...
  customer_name=:cust,order_date=:orderdate,subtotal=:stotal,
  tax=:tax,discount=:disc,total=:total,paid=:paid,
  due=:due,payment_type=:ptype WHERE invoice_id=$id");
  ...
  // 2nd insert query for tbl_invocie_details
  $invoice_id = $pdo->lastInsertId();
  if($invoice_id != null){
    for($i=0; $i<count($arr_productid); $i++){
      //5) Write select query for tbl_product table to get out stock value
      $selectpdt=$pdo->prepare("SELECT * FROM tbl_product WHERE
       pid='".$arr_productid[$i]."'");
       $selectpdt->execute();
 ...
 //6) Write update query for tbl_product table to update stock values
   ...
   $update = $pdo->prepare("UPDATE tbl_product SET pstock = '$rem_qty'
    ...
    //7) Write insert query for tbl_invoice_details for insert new records
      ...
      tbl_invoice_details(invoice_id,product_id,product_name,qty,price,order_date)
...
<div class="content-wrapper">
    ...
    <section class="content container-fluid">
                ...
                <table id="producttable" class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th>Total</th>        
                            <th>
                  <center> <button type="button" name="add" class="btn btn-info btnadd btn-sm">
        ...
        <tr>
          <?php 
 echo '<td><input type="hidden" class="form-control pname" name="productname[]" 
  value="'.$row_product['pname'].'" readonly></td>';
 ...
 echo '<td><input type="text" class="form-control stock" value="'.$row_product['pstock'].'" name="stock[]" readonly></td>';
 echo '<td><input type="text" class="form-control price" value="'.$row_product['saleprice'].'" name="price[]"  readonly></td>';
 echo'<td><input type="number"min ="1" class="form-control qty" value="'.$item_invoice_details['qty'].'" name="qty[]" required></td>';
 ...
 echo '<td><center><button type="button" name="remove" class="btn btn-danger btnremove btn-sm">
           <span class="glyphicon glyphicon-remove"></span></button><center></td></center>';
    ...
    <div class="form-group">
     <label>
<input type="radio" name="rb" class="minimal" value="Cash"<?php echo ($payment_type == 'Cash')? 'checked':'' ?>> CASH
    </label>
    <label>
      <input type="radio" name="rb" class="minimal" value="Card"<?php echo ($payment_type == 'Card')? 'checked':'' ?>> CARD
    </label>
    <label>
      <input type="radio" name="rb" class="minimal" value="Check"<?php echo ($payment_type == 'Check')? 'checked':'' ?>> CHECK
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      $(document).on('click', '.btnadd', function(){
        ...
        html += `<td><center><button type="button" name="remove" class="btn btn-danger btnremove btn-sm">
                  <span class="glyphicon glyphicon-remove"></span></button></center></td>`
      ...
      $('#producttable').delegate(".qty","keyup change", function(){
        ...
        if((quantity.val()-0)>(tr.find(".stock").val()-0)){
          swal("WARNING!","SORRY! This much of qunatity is not available","warning");
          quantity.val(1);
